...
FUNCTION dok_ensure_range {
  PARAMETER targetship, distance, speed.
  LOCK relativePosition TO SHIP:POSITION - targetship:POSITION.
  LOCK departVector TO (relativePosition:normalized * distance) - relativePosition.
  LOCK relativeVelocity TO SHIP:VELOCITY:ORBIT - targetship:VELOCITY:ORBIT.
  clearscreen.
...
FUNCTION dok_dock {
	PARAMETER targetship, targetPort, dockingPortTag.
	SET dockingPort TO dok_get_port(dockingPortTag).
	dockingPort:controlfrom.
	RCS ON.
	//print "Ensuring range".
	//dok_ensure_range(targetship, 100, 1).
	//print "At range".
	//wait 1.
	//dok_kill_relative_velocity(targetship).
	//clearscreen.
	//print "Speed killed".
	//wait 1.
	//clearscreen.
    ...
    print "Performing approach from " + (targetship:distance).
